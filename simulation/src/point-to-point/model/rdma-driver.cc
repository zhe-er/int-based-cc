...
//set controller
void
RdmaDriver::MyController (Ptr<ControlDecider> controller)
{
...
void RdmaDriver::AddQueuePair(uint64_t size, uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint32_t win, uint64_t baseRtt, Callback<void> notifyAppFinish){
	...
	uint64_t mykey = ((uint64_t)dip.Get() << 32) | ((uint64_t)sport << 16) | (uint64_t)pg;
	//to compute the fairness of competition
	driver_controller->cc_round = driver_controller->cc_round + 1;
}
