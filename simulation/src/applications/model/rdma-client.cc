...
//set controller
void
RdmaClient::MyController (Ptr<ControlDecider> controller)
{
...
void RdmaClient::StartApplication (void)
{
  ...
  uint64_t mykey = ((uint64_t)m_dip.Get() << 32) | ((uint64_t)m_sport << 16) | (uint64_t)m_pg;
  client_controller->SetKey(mykey, m_size);
