...
namespace ns3 {
		...
		Queue()
	{
		...
		//probability of multiqueues
		m_vecWeight.push_back(0.5);
		m_vecWeight.push_back(0.3);
		m_vecWeight.push_back(0.2);
		...
		BEgressQueue::DoEnqueue(Ptr<Packet> p, uint32_t qIndex)
	{
		...
		if (p->FindFirstMatchingByteTag (ipv4PriorityTag)) 
      	{
			qIndex = ipv4PriorityTag.GetPriorityTag()+1;
		}
		...
		BEgressQueue::DoDequeueRR(bool paused[]) //this is for switch only
	{
		...
		else
		{
			...
			//dequeue packets based on probability
			//other queues
			std::vector<uint32_t> vecIndex;
			int m_numPriority = 3;
			for (uint32_t i = 1; i < m_numPriority+1; i++)
			{
